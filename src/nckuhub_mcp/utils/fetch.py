...
async def fetch_all_course() -> list[dict]:
    data = await fetch(URL + "course")
    obj = json.loads(data)
    return obj["courses"]


async def fetch_course(course_id: str) -> dict:
    data = await fetch(URL + "course" + "/" + course_id)
    return json.loads(data)


async def fetch_all_dpmt() -> list[dict]:
    data = await fetch(URL + "course" + "/allDpmt")
    return json.loads(data)


async def fetch_all_course_prev() -> list[dict]:
